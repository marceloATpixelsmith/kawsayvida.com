...
function isSameOriginRequest(request: Request): boolean {
  ...
  return Boolean(origin) && hostname(origin ?? '') === requestHostname(request)
}
...
function validFieldValue(value: unknown): value is string | boolean | string[] {
  if (typeof value === 'boolean') return true
  if (typeof value === 'string') return value.length <= MAX_FIELD_VALUE_LENGTH
  return Array.isArray(value)
    && value.length <= MAX_ARRAY_ITEMS
    && value.every((item) => typeof item === 'string' && item.length <= MAX_FIELD_VALUE_LENGTH)
}

function isValidFields(value: unknown): value is Record<string, string | boolean | string[]> {
...
function isValidPayload(value: unknown): value is AttemptPayload {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false
  ...
  return (payload.form === 'contact' || payload.form === 'registration')
    ...
    && (payload.lang === undefined || (typeof payload.lang === 'string' && payload.lang.length <= MAX_LANG_LENGTH))
}

...
These instructions are the shared default for repositories owned by this account. Follow more-specific repository or task instructions when they exist.
...
- When asked to provide code in chat, provide complete usable code rather than omitted sections or placeholder fragments.

## Validation before completion

Determine the commands that actually exist in the repository and run the applicable checks after changes. Prefer this order when available:
...
Do not assume all four scripts exist. Inspect `package.json` first.
...
- For UI changes, inspect the affected pages at relevant responsive sizes when the environment provides browser or screenshot capabilities.

## Git and scope discipline

- Keep changes limited to the requested task.
- Do not rewrite unrelated files simply to reformat or modernize them.
- Do not force-push, rewrite history, or amend existing commits unless explicitly instructed.
- Do not modify generated deployment state or production configuration unless the task requires it.
- Before finishing, review the diff for accidental changes, secrets, generated junk, duplicate lockfiles, and unrelated formatting churn.

## Working in Claude's execution environment

- Use the tools and permissions actually available in the current Claude environment; do not assume local-machine access, browser access, network access, deployment credentials, or third-party service credentials.
- When a task depends on something unavailable in the environment, complete the repository work that can be completed safely and identify the specific remaining verification rather than fabricating a result.
- Prefer inspecting the repository and executing its own documented commands over guessing how the project works.
